  ...
  return (
    <>
      <div
        ...
        className={`py-15 px-10 md:p-25 w-full bg-green-background cursor-pointer rounded-soft md:rounded-normal border-solid ${borderColor} border group`}>
        <div className="flex justify-between">
          <SwapStepSwapper step={step} />
          <button
            onClick={() => setIsOpen(!isOpen)}
            className="w-[26px] h-[26px] hidden md:flex text-neutral-400 group-hover:text-hoverIcon border-primary-500 p-5 bg-transparent group-hover:bg-transparent items-center justify-center cursor-pointer rounded-full">
            {isOpen ? (
